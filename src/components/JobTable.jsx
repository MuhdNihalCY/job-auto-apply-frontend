...
export default function JobTable({ refreshKey, onEdit, onRefresh }) {
  ...
  const statuses = ["All", "Pending", "Queued", "Sent", "Failed", "Skip"];

  const filtered = jobs.filter((j) => {
    const matchStatus =
      filterStatus === "All" || j.send_status === filterStatus;
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200">
      {/* Toolbar */}
      <div className="px-4 py-3 border-b border-gray-100 flex flex-wrap gap-2 items-center justify-between">
              ...
              filtered.map((job, i) => (
                <tr key={job.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-4 py-3 text-gray-500 text-xs">
                    {job.apply_email || <span className="italic">—</span>}
                  </td>
                  <td className="px-4 py-3">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                        statusBadge[job.send_status] ?? "bg-gray-100 text-gray-500"
                      }`}
                    >
                      {job.send_status || "—"}
                    </span>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      ...
                      <button
                        ...
                        className="text-xs px-2 py-1 rounded border border-red-100 hover:bg-red-50 text-red-500 transition-colors"
                      >
                        Delete
                      </button>
                    </div>
